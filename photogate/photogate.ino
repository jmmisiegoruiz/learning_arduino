...
CircularBuffer<float,3> eventsBuffer;
	...
	if (photoGate != lastState)  // if there is a change, 
		{
			...
			Serial.print(DELIM);
			Serial.print(photoGate); // print out the state of the gate
			eventNum++; // increment the eventNum

      if (photoGate == HIGH) {
        eventsBuffer.push(eventTime);
        float period = (eventsBuffer.last() - eventsBuffer.first())/1000.0;
        float gravity = (4 * sq(PI) * 0.13)/sq(period);
